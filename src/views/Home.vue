<template>
  <div id="app">
    ...
    <el-form>
      <el-form-item label="username">
        <el-input
          ...
          placeholder="Meet old friend? Insert their UserID / Username"
        ></el-input>
      </el-form-item>
      <el-form-item>
        <el-select
          ...
          placeholder="Filter"
        >
          <el-option label="Male" value="sir"></el-option>
          <el-option label="Female" value="miss"> </el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button size="small" style="float:left" @click="search">Search</el-button>
      </el-form-item>
    </el-form>
    ...
    <div class="app-table">
      <el-table :data="onlineClients" style="width: 100%">
        <el-table-column prop="userId" label="UserID"></el-table-column>
        <el-table-column prop="username" label="UserName">
        </el-table-column>
        <el-table-column prop="sex" label="Gender"></el-table-column>
        <el-table-column prop="zoomLink" label="ZoomLink"></el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  mounted() {
    this.headerUrl = window.localStorage.getItem("header");
  ...
  methods: {
    search() {
      ...
      for (let i = 0; i < this.onlineClients.length; i++) {
        if (
          ...
          this.queryForm.username === this.onlineClients[i].username
        ) {
          tmpData.push(this.onlineClients[i]);
        } else if (
          this.queryForm.sex !== "" &&
          this.queryForm.sex === this.onlineClients[i].sex
        ) {
          console.log(this.queryForm.sex);
          tmpData.push(this.onlineClients[i]);
        }
      }
      this.onlineClients = tmpData;
    },
    // 获取url参数
    getUrlKey(name) {
      return (
    ...
    startAction(callback) {
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          console.log("Get the video stream");
        })
        .catch((err) => {
          console.log(err);
        });
    ...
    handleRemoteMediaStreamAdded(pc, event) {
      ...
      console.log(`${pc.other.username} sends you a video call request`);
    },
    // 发送消息
    ...
    signalingMessageCallback(message) {
      ...
      if (message.type === "offer") {
        ...
        console.log("Receive a message from others");
        console.log("pc", pc);
      ...
      if (!this.localStream) {
        this.startAction(() => {
          this.socket.emit("interact", {
            from: { username: this.username, userId: this.socket.id },
            to: user,
          });
          console.log(`${this.username} sents the video call invitation to ${user.username}`);
        });
      } else {
        this.socket.emit("interact", {
          from: { username: this.username, userId: this.socket.id },
    ...
    joinRoom() {
      ...
      this.socket.on("join", (data) => {
        console.log({ msg: data.username + "User in", type: "sys" });
      });
      // 自己加入成功
      ...
      this.socket.on("leave", (data) => {
        console.log({ msg: data.username + "left", type: "sys" });
      ...
      this.socket.on("pc message", (data) => {
        console.log("You received a message from PC end", data);
        this.signalingMessageCallback(data);
      });
      // 收到别人的要求视频互动的私信
      this.socket.on("interact", (data) => {
        this.$confirm(
          `${data.from.username} wants eat with you, are you going to accept it?`,
          "Infomation",
          {
      ...
      this.socket.on("agree interact", (data) => {
        this.$message.success(`${data.to.username} accepts your video call invitation`);
        this.pcMsgTo = data.to;
        console.log(`${data.to.username} accepts your video call invitation`);
        this.createPeerConnection(true, data);
      });
      // 对方拒绝了和你视频互动
      this.socket.on("refuse interact", (data) => {
        this.$message.warning(`${data.to.username} rejects your video call invitation`);
        console.log(`${data.to.username} rejects your video call invitation`);
        this.closeConnection();
      });
      // 监听到对方结束互动
      this.socket.on("stop interact", (data) => {
        let part = data.from;
        this.$message.info(`${part.username} stops the video call`);
      ...
      this.socket.on("close_disconnect", (data) => {
        ...
        if (data[0]) {
          this.$message.warning(`You stops the video call from ${data[0].username}`);
        }
        this.closeConnection();
      });
    },
    ...
    closeLocalMedia() {
      ...
      if (
        ...
        this.$refs.localVideo.srcObject.getTracks()
      ) {
      ...
      if (
        this.biPeersList.other &&
        this.$refs["remoteVideo" + this.biPeersList.other.userId][0]
          .srcObject &&
        this.$refs[
          "remoteVideo" + this.biPeersList.other.userId
        ][0].srcObject.getTracks()
      ) {
        this.$refs["remoteVideo" + this.biPeersList.other.userId][0].srcObject
          .getTracks()
          .forEach((track) => {
            track.stop();
          });
